...
def test_send_amendment_review_invite_links():
    app = create_app()
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///:memory:'
    app.config['MAIL_SUPPRESS_SEND'] = True
    app.config['TOKEN_SALT'] = 's'
    with app.app_context():
        ...
        meeting = Meeting(title='AGM')
        db.session.add(meeting)
        member = Member(name='Joe', email='j@example.com', meeting_id=1)
        db.session.add(member)
        db.session.commit()
        with app.test_request_context('/'):
            with patch.object(mail, 'send') as mock_send:
                send_amendment_review_invite(member, meeting, test_mode=False)
                mock_send.assert_called_once()
                sent = mock_send.call_args[0][0]
                assert '/review/' in sent.body
